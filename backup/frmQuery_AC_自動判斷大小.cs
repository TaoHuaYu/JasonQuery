...
/* 重建方案前，再次確認此表單物件的位置、大小
   【editor】Width=755
   【pnlFindReplace】Left=430
*/
...
namespace JasonQuery
{
    public partial class frmQuery : Form
    {
        ...
        private string _UnfreezeColumnName = "";
        ...
        private DataTable _dtNull = new DataTable(); //查詢前的空 Table，讓 Grid 呈現用
        private DataTable _dtSchemaTable_Export = new DataTable(); //承接查詢時所儲存的 SchemaTable
        ...
        private bool drag;
        private int x, y; //for Panel moving: Find/Replace
        private int _iPeriodPos = 0; //記住顯示欄位下拉清單時的游標所在位置
        private int _iPeriodPos2 = 0; //記住顯示欄位下拉清單時的游標所在位置 (如果 c1GridAC 是顯示狀態，記住最新的游標所在)
        private DataTable _dtAC4Period = new DataTable();
        private bool _bKeyCodeCtrlJ = false; //記住是否按下 Ctrl+J (KeyUp 無法偵測 Ctrl+J)
        private int _iSpacePos = 0; //記住顯示欄位下拉清單時的游標所在位置
        private int _iSpacePos2 = 0; //記住顯示欄位下拉清單時的游標所在位置 (如果 c1GridAC 是顯示狀態，記住最新的游標所在)
        ...
        private DataTable _oDataTable = new DataTable();
        ...
        private DataTable _dtSchemaTable = new DataTable();
        private Point _dragStartPosition = Point.Empty; //20190930 for TabControl drag & drop
        private DataTable _ooDataTable = new DataTable(); //20191215 for 查詢用的 DataTable
        private int _iSquiggleNum = 11; //20191224 波浪底線
        ...
        private void BuildAutocompleteMenu()
        {
            ...
            var sTemp = (MyLibrary.bACBuiltInKeywords == false ? "" : MyLibrary.sKeywordsBuiltInKeywords) + " " + (MyLibrary.bACBuiltInFunctions == false ? "" : MyLibrary.sKeywordsBuiltInFunctions) + " " + (MyLibrary.bACUserDefinedKeywords == false ? "" : MyLibrary.sKeywordsUserDefinedKeywords);
            sTemp += (MyLibrary.bACUserDefinedTables == false ? "" : MyLibrary.sKeywordsUserDefinedTables) + " " + (MyLibrary.bACUserDefinedFunctions == false ? "" : MyLibrary.sKeywordsUserDefinedFunctions) + " " + (MyLibrary.bACUserDefinedViews == false ? "" : MyLibrary.sKeywordsUserDefinedViews) + " " + (MyLibrary.bACUserDefinedTriggers == false ? "" : MyLibrary.sKeywordsUserDefinedTriggers);
            sTemp = sTemp.Replace("  ", " ").Replace(" ", "`") + "`";

            if (sTemp != "")
            {
                bValue = true;
                var keywords = sTemp.Split(new[] { "`" }, StringSplitOptions.RemoveEmptyEntries);
            ...
            if (MyLibrary.bACUserDefinedSnippets)
            {
                var dtTemp = MDBCommon.ExecQuery("SELECT * FROM SystemConfig WHERE DomainUser = '" + MyGlobal.sDomainUser + "' AND MPID = " + MyGlobal.sDBMotherPID + " AND AttributeKey = 'AutoCompleteConfig' AND AttributeName = 'Snippets'");
                ...
                if (dtTemp.Rows.Count > 0)
                {
                    ...
                    {
                        sTemp += dtTemp.Rows[iRow]["AttributeText"].ToString() + "`";
                    }

                    var snippets = sTemp.Split(new[] { "`" }, StringSplitOptions.RemoveEmptyEntries);

                    items.AddRange(snippets.Select(item => new SnippetAutocompleteItem(item) {ImageIndex = 1}));
                }
            }
        ...
        {
            MyGlobal.ApplyLanguageInfo(this); //ApplyLanguageInfo
            ...
            _sLangText = MyGlobal.GetLanguageString("Find...", "form", Name, "object", "cboFindBox", "ToolTipText");
            SendMessage(cboFindBox.Handle, EM_SETCUEBANNER, 0, _sLangText);
            _sLangText = MyGlobal.GetLanguageString("Replace...", "form", Name, "object", "cboReplaceBox", "ToolTipText");
            SendMessage(cboReplaceBox.Handle, EM_SETCUEBANNER, 0, _sLangText);
            ...
            else
            {
                ...
                cboFind.VisualStyle = C1.Win.C1Input.VisualStyle.Office2007Blue;
            ...
            chkShowColumnType.Location = new Point(cboFindGrid.Left + cboFindGrid.Width + 152, chkShowColumnType.Top);
            chkShowColumnType.Checked = MyLibrary.bGridShowColumnDataType;

            chkRawDataMode.Location = new Point(chkShowColumnType.Left + chkShowColumnType.Width + 1, chkRawDataMode.Top);
            ...
            cboFind.Location = new Point(Convert.ToInt16(tsSeparator.Tag.ToString()) + (MyLibrary.bShowSaveAsButton == false ? 0 : btnSaveAs.Width) + lblFind.Width - 32, cboFind.Top);
            ...
            _cMenuMessageEditor.Items[0].Click += delegate
            {
                ...
                txtMessage.SelectionEnd = txtMessage.Text.Length;
            };
            ...
            _cMenuMessageEditor.Items[2].Click += delegate
            {
                ...
                {
                    Clipboard.Clear();
                    txtMessage.Copy(ScintillaNET.CopyFormat.Text | ScintillaNET.CopyFormat.Rtf | ScintillaNET.CopyFormat.Html);
                }
                else
                {
                    txtMessage.Copy();
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            ...
            if (MyLibrary.bDarkMode && MyGlobal.dtSchema == null) //如果是深色模式，先置換 Editor 底色 (避免底色反白停留過久)
            {
                ...
                txtMessage.CaretLineBackColor = ColorTranslator.FromHtml(MyLibrary.sColorEditorBackground);
                txtMessage.Styler = new SqlStyler();
            }
            ...
            tsSeparator.Tag = cboFind.Left.ToString(); //記住 Left 值
            ...
            cboActionScope.Text = MyGlobal.GetLanguageString("Current Document", "form", Name, "object", "cboActionScope", "Text");
            ...
            c1TrueDBGrid1.KeyUp += Detect_KeyUp;
            ...
            {
                //ScintillaNET 元件，要用以下寫法；無法像 TextBox/RichTextBox 可以直接在屬性設定
                autocompleteMenu1.TargetControlWrapper = new ScintillaWrapper(editor);
                autocompleteMenu1.MinFragmentLength = MyLibrary.iACMinFragmentLength;
            ...
            txtMessage.TabWidth = MyGlobal.iTabWidth;
            ...
            if (MyGlobal.dtSchema != null)
            {
                MyGlobal.UpdateSchemaData(c1GridSchemaBrowser); //依資料庫類別，進行 Grid's Group、欄位隱藏、相關設定...
            }
            else
            {
                ...
                switch (MyGlobal.sDataSource)
                {
                    ...
                    case @"PostgreSQL":
                        ...
                        break;
                    case @"SQL Server":
                        MyGlobal.UpdateSchemaData_SQLServer(c1GridSchemaBrowser); //Form_Load, SQL Server
                        break;
                    case @"MySQL":
            ...
            {
                if (MyGlobal.sDBConnectionDatabase == "")
                {
            ...
            AutoResizeGridColumnWidth(); //for Schema Browser & Auto Replace
            ...
            pnlFindReplace.Location = new Point(Width - pnlFindReplace.Width, pnlFindReplace.Top);
        }
        ...
        private int c1GridAC4Period_Filter(string sCondition)
        {
            ...
            c1GridAC4Period1.DataSource = dataView;
        ...
        private int c1GridAC4Space_Filter(string sCondition)
        {
            ...
            c1GridAC4Space1.DataSource = dataView;
        ...
        private void CreateAndGetARInfoTable(DataTable dt = null, bool bReserve = false)
        {
            ...
            {
                dt = MDBCommon.ExecQuery("SELECT PID, AttributeValue FROM SystemConfig WHERE DomainUser = '" + MyGlobal.sDomainUser + "' AND MPID = " + MyGlobal.sDBMotherPID + " AND AttributeKey = 'AutoReplaceConfig' AND AttributeName = 'AutoReplace'");
        ...
        private void ApplyEditorSetting()
        {
            ...
            txtMessage.SetSelectionBackColor(true, ColorTranslator.FromHtml(MyLibrary.sColorSelectedTextBackground)); //變更選取區塊的底色
            txtMessage.CaretLineBackColor = ColorTranslator.FromHtml(MyLibrary.sColorCurrentLineBackground);
            ...
            //套用 Auto Complete Color
            autocompleteMenu1.Colors.ForeColor = ColorTranslator.FromHtml(MyLibrary.sACForeColor);
            autocompleteMenu1.Colors.BackColor = ColorTranslator.FromHtml(MyLibrary.sACBackColor);
            autocompleteMenu1.Colors.SelectedForeColor = ColorTranslator.FromHtml(MyLibrary.sACSelectedForeColor);
            autocompleteMenu1.Colors.SelectedBackColor = ColorTranslator.FromHtml(MyLibrary.sACSelectedBackColor);
            autocompleteMenu1.Colors.SelectedBackColor2 = ColorTranslator.FromHtml(MyLibrary.sACSelectedBackColor);
            autocompleteMenu1.Colors.HighlightingColor = ColorTranslator.FromHtml(MyLibrary.sACBackColor);
        }
        ...
        private void RemoveComment() //去除 -- 註解
        {
            ...
            for (var i = 0; i < parts.Length; i++)
            {
                ...
                {
                    if (sTemp.Substring(j, 1) == " ") continue;
        ...
        private void GetNewStartAndEnd(string sText, int iStart, int iEnd, ref int iStart2, ref int iEnd2, ref int iNoneSpaceStart, string sMode = "")
        {
            ...
            if (sMode.ToUpper() != "ADDCOMMENT") return;
        ...
        {
            var bValue = editor.SelectedText != ""; //判斷是否有選取文字，決定功能表項目可不可用
            ...
            _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.Find].Enabled = editor.Text != "";
            _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.Replace].Enabled = editor.Text != "";
            _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.SelectAll].Enabled = editor.Text != "";
            _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.SelectCurrentBlock].Enabled = editor.Text != "";
        ...
        private void ApplyEditorMenu() //套用 Editor 右鍵功能表
        {
            ...
            _lstMenuEditor.Add(_sLangText);
            _lstMenuEditor.Add("-");
            _sLangText = MyGlobal.GetLanguageString("Find", "form", Name, "menueditor", "Find", "Text");
            _lstMenuEditor.Add(_sLangText);
            _sLangText = MyGlobal.GetLanguageString("Replace", "form", Name, "menueditor", "Replace", "Text");
            _lstMenuEditor.Add(_sLangText);
            _lstMenuEditor.Add("-");
            _sLangText = MyGlobal.GetLanguageString("Select All", "form", Name, "menueditor", "SelectAll", "Text");
            _lstMenuEditor.Add(_sLangText);
            ...
            _cMenuQueryEditor.Items.Add(_lstMenuEditor[(int)_eMenuQueryEditor.Find]);
            _cMenuQueryEditor.Items.Add(_lstMenuEditor[(int)_eMenuQueryEditor.Replace]);
            ...
            ((ToolStripMenuItem) _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.Find]).ShortcutKeys = Keys.Control | Keys.F;
            ((ToolStripMenuItem) _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.Replace]).ShortcutKeys = Keys.Control | Keys.H;
            ...
            {
                if (MyLibrary.bCopyAsHTML && editor.Tag.ToString() == "")
                {
                    try
                    {
                        Clipboard.Clear();
            ...
            _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.Find].Click += delegate
            {
                ShowFindAndReplace("F"); //右鍵選單 Find, ShowFindAndReplace
            };

            _cMenuQueryEditor.Items[(int)_eMenuQueryEditor.Replace].Click += delegate
            {
                ShowFindAndReplace("R"); //右鍵選單 Replace, ShowFindAndReplace
            };
        ...
        private void SortDataGrid(int iCol, bool bByNumber = false)
        {
            ...
            var _sortDown = new Bitmap(a.GetManifestResourceStream(a.GetName().Name + ".Files.SortDn.bmp"));
            var _sortUp = new Bitmap(a.GetManifestResourceStream(a.GetName().Name + ".Files.SortUp.bmp"));
            _sortDown.MakeTransparent(Color.Red);
            ...
            var sCondition = dc.DataColumn.DataField + " " + sSortMode;

            var dt = new DataTable();
            dt = ((DataTable)c1Grid.DataSource).Copy();
            ...
            dc.HeadingStyle.ForegroundImage = sSortMode == "Asc" ? _sortUp : _sortDown;
        ...
        {
            var sResult = "";
            var sSelectedText = editor.Tag.ToString() == "" ? editor.SelectedText : editor.Tag.ToString();
        ...
        /// <returns></returns>
        private bool IsWithSql(string sSql) //判斷 With 子句屬於 Select 還是 Update/Delete/Insert？
        {
            var bResult = true;
            var sTempSql = "";
        ...
        private string GetFormattedSql(string sSql, bool bToUpper = true)
        {
            //去除「區段註解」
            for (var i = 0; i < 100; i++)
            {
            ...
            //重新組成單獨一列的 SQL 指令
            sSql = parts.TakeWhile((t, i) => i <= 300).Aggregate("", (current, t) => current + (t == "" ? "" : t + " "));
        ...
        private static string GetParametersInfo(string sSql, out string sResultAll)
        {
            ...
            const string sFrontChar = "`(`)`=` `"; //變數：冒號前一個字元，判斷是否為變數
            const string sEndChar = "` `-`/`*`%`;``.,`(`)`&`#`@`!`^`+`|`'`?`<`>`~`=`{`}`[`]`"; //參數的結尾符號：- 或 / 或 ; 或 ( 或 ) 或 ,判斷是否為參數的結尾
        ...
        private void CheckFileDateTimeAndExist()
        {
            ...
            if (sTemp != "" && File.Exists(sTemp) == false)
            {
            ...
            //檢查是否有外部編輯器編輯此檔案
            else if (sTemp != "" && sSavedDateTime != "") //sTemp=""，表示尚未存檔，不用處理
            {
                sFilename = Path.GetFileName(sTemp);
                ...
                try
                {
                    ...
                    {
                        LoadFile(sTemp, out bLargeFile); //CheckFileDateTimeAndExist
        ...
        private void ExecuteQuery(string sSql = "", bool bNextPage = false)
        {
            ...
            var sQueryText = sSql == "" ? editor.SelectedText : sSql;
            ...
            if (sParametersDistinct != "")
            {
                ...
                else
                {
                    ...
                    using (var myForm = new frmParameters())
                    {
                        ...
                        var dtData = MDBCommon.ExecQuery(sTemp);
                        ...
                        dtData = MDBCommon.ExecQuery(sTemp);
            ...
            if (bQuery == false)
            {
                ...
                {
                    if (bIgnoreCommitRollback)
                    {
                        btnQuery.AccessibleDescription = "query";
                    }
                    else
                    {
                        btnQuery.AccessibleDescription = "nonquery";
                    }
        ...
        private bool IsQuery(string sSql, out int iQueryCount, out bool bCommitRollback, out bool bExtended, out bool bIgnoreCommitRollback)
        {
            var bResult = false;
            iQueryCount = 0;
        ...
        private void btnCancelQuery_Click(object sender, EventArgs e)
        {
            ...
            lblInfo.Text = "Execution aborted...";
            ...
            try
            {
                switch (MyGlobal.sDataSource)
                {
                    case @"Oracle":
                        ...
                        sMsg = sMsg == "" ? "" : "InterruptQueryMsg: " + sMsg;
                        break;
                    case @"PostgreSQL":
            ...
            switch (MyGlobal.sDataSource)
            {
                case "Oracle" when MyGlobal.sGlobalTemp.IndexOf("sqlexecuteerrorpos", StringComparison.Ordinal) == -1:
                    ...
                    catch (Devart.Data.Oracle.OracleException ex)
                    {
                        ...
                        var sErrCode = ex.Code.ToString() == "" ? "" : "ErrorCode: " + ex.Code + "\r\n";
                ...
                else
                {
                    foreach (Control tab in c1DockingTab1.TabPages)
                    {
                        ...
                        foreach (var ctrlTab in tabPage.Controls)
                        {
                            ...
                            ((C1TrueDBGrid)ctrlTab).KeyUp += Detect_KeyUp;
        ...
        private void ArrangeDataTable_Oracle(C1TrueDBGrid c1Grid, DataTable dtData, DataTable dtSchemaTable, bool bSort = true, bool bForceResize = false)
        {
            ...
            _dtSchemaTable_Export = dtSchemaTable;
            ...
            try
            {
                for (var i = 0; i < dtData.Rows.Count; i++)
                {
                    ...
                    for (var j = 0; j < dtData.Columns.Count; j++)
                    {
                        ...
                        {
                            case "DATETIME":
                                if (sTemp.Trim() != "" && "`<null>`{null}`(null)`".ToUpper().Contains("`" + sTemp.ToUpper() + "`") == false)
                                {
            ...
            if (btnCancelQuery.Tag.ToString() == "Cancel") return;
            ...
            //Auto Size
            if (iTotalRows <= 0 || !chkSize.Checked && !bForceResize) return;
                ...
                for (var i = 0; i < dtData.Rows.Count; i++)
                {
                    ...
                    for (var j = 0; j < dtData.Columns.Count; j++)
                    {
                        ...
                        if (dtSchemaTable != null && dtSchemaTable.Rows.Count > 0)
                        {
                            ...
                            else if (sFormatDataType == "time with time zone")
                            {
                                if (sTemp.Trim() != "" && "`<null>`{null}`(null)`".ToUpper().Contains("`" + sTemp.ToUpper() + "`") == false)
                                {
        ...
        private void btnCommit_Click(object sender, EventArgs e)
        {
            ...
            switch (MyGlobal.sDataSource)
            {
                case @"Oracle":
                    ...
                    sValue += (sValue == "" ? "Commit executed at " : " ") + DateTime.Now.ToString(MyLibrary.sDateFormat + " HH:mm:ss");
            ...
            //22020423
            //////////ConnectToDatabase(); //from frmQuery's btnCommit_Click()
        ...
        private void btnRollback_Click(object sender, EventArgs e)
        {
            ...
            switch (MyGlobal.sDataSource)
            {
                ...
                case @"Oracle":
                    ...
                    sValue += (sValue == "" ? "Rollback executed at " : " ") + DateTime.Now.ToString(MyLibrary.sDateFormat + " HH:mm:ss");
                    MyGlobal.oOracleReader.oDisconnect();
                    break;
            ...
            //22020423
            //////////ConnectToDatabase(); //from frmQuery's btnRollback_Click()
        ...
        private static void ConnectToDatabase()
        {
            switch (MyGlobal.sDataSource)
            {
                ...
                case @"MySQL":
                    ...
                    break;
                case @"SQLite":
                    MyGlobal.oSQLiteReader.ConnectTo(MyGlobal.sDBConnectionString);
                    break;
                case @"SQLCipher":
                    MyGlobal.oSQLCipherReader.ConnectTo(MyGlobal.sDBConnectionString);
                    break;
            }
        }
        ...
        private void CheckEditorContent()
        {
            ...
            lblFind.Enabled = bValue;
            cboFind.Enabled = bValue;
        ...
        private void cboFind_Enter(object sender, EventArgs e)
        {
            tsEditor.BackColor = _cToolstripFocused;
        }
        ...
        {
            var bValue = cboFind.Text.Trim() != "";

            btnFindNext.Enabled = bValue;
            btnFindPrevious.Enabled = bValue;
            btnCount.Enabled = bValue;
            btnHighlightAll.Enabled = bValue;
        }

        private void cboFind_DropDown(object sender, EventArgs e)
        {
            LoadFindList("Editor", cboFind); //cboFind_DropDown
        }

        private void cboFind_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar != 13 || cboFind.Text.Trim() == "")
            {
                return;
            }

            UpdateFindList(cboFind); //cboFind_KeyPress

            //可能透過「貼上」
            cboFind.Tag = CountEditor(cboFind).ToString(); //cboFind_KeyPress, 按下 Enter, 統計出現次數，但不顯示
        ...
        private void cboFind_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            btnHighlightAll.Enabled = true;

            cboFind.Tag = CountEditor(cboFind, false).ToString(); //cboFind_SelectedIndexChanged, 統計出現次數，但不顯示
        }
        ...
        private void cboFind_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                bValue = true;
                cboFind.Tag = ""; //改成每次輸入都清空，按下「Next」/「Previous」/「Highlight」按鈕時再去統計

                //20201209 每輸入一個字元就統計一次，如果SQL內容很多，就會嚴重 lag
                //iCount = CountEditor(cboFind); //cboFind_TextChanged, 統計出現次數，但不顯示
            }
            
            //cboFind.Tag = iCount.ToString();
        ...
        {
            if (cboFind.Tag.ToString() == "")
            {
                cboFind.Tag = CountEditor(cboFind, false, !btnMatchCase.Visible).ToString(); //btnFindPrevious_Click
            }
            
            FindPrevious4Editor(cboFind, btnFindPrevious.ToolTipText);
        }

        private void FindPrevious4Editor(C1ComboBox cboFind, string sFind, bool bMatchCase = false, bool bWholeWord = false)
        {
            if (cboFind.Text.Trim() == "")
            {
                cboFind.Text = "";
            ...
            if (cboFind.Tag.ToString() == "0")
            {
                _sLangText = MyGlobal.GetLanguageString("Can't find the text", "form", Name, "msg", "CantFindText", "Text");
                SetLabelInfoEditor(sFind + ": " + _sLangText + " \"" + cboFind.Text + "\".", Color.Red);
                return;
            }

            SetLabelInfoEditor("", Color.Black);

            UpdateFindList(cboFind); //btnFindPrevious_Click

            if (FindPrevious(cboFind, false, bMatchCase, bWholeWord) == false) //btnFindPrevious_Click: 1st
            {
                //往下找不到資料，而且不是在第一格位置，再從頭開始找一次
                FindPrevious(cboFind, true, bMatchCase, bWholeWord); //btnFindPrevious_Click: 2nd
            }
        }

        private bool FindPrevious(C1ComboBox cboFind, bool bFindAgain = false, bool bMatchCase = false, bool bWholeWord = false)
        {
            var sAllText = editor.Text;
            var sSearchText = bMatchCase == false ? cboFind.Text.ToUpper() : cboFind.Text;
            bool bResult;
            var bWholeWordStart = false;
            var bWholeWordEnd = false;
            var iStartOriginal = bFindAgain ? sAllText.Length : editor.SelectionStart;
            var iEndOriginal = bFindAgain ? sAllText.Length : editor.SelectionEnd;
            var iStart = 0;
            var iEnd = 0;
            ...
            {
                SaveFindList("Editor", cboFind.Text); //FindPrevious
            }
            ...
            for (var i = iStartOriginal; i >= 0; i--)
            {
                ...
                var sLetter = bMatchCase == false ? letter.ToString().ToUpper() : letter.ToString();

                if (sLetter != sSearchText.Substring(sSearchText.Length - 1, 1)) continue;
                ...
                for (var j = sSearchText.Length - 1; j > 0; j--)
                {
                    ...
                    try
                    {
                        letter = array[i - iPos - 1];
                        sLetter = bMatchCase == false ? letter.ToString().ToUpper() : letter.ToString();

                        if (bMatchFound && j < sSearchText.Length && sLetter == sSearchText.Substring(j - 1, 1))
                        {
                            bMatchFound = true;
                        }
                        else
                ...
                if (i - 1 - 1 <= 0 || IsRealSymbol(editor.Text.Substring(i - 1 - 1, 1)))
                {
                    bWholeWordStart = true;
                }

                if (bWholeWordStart && (iEnd + 1 == editor.TextLength || IsRealSymbol(editor.Text.Substring(iEnd + 1, 1))))
                {
                    bWholeWordEnd = true;
                }

                if (!bWholeWordStart || !bWholeWordEnd) continue;
            ...
            if (iStart >= 0 && iEnd > 0)
            {
                editor.Focus();
                editor.SelectionStart = iStart;
                editor.SelectionEnd = iEnd + 1;
                editor.ScrollCaret();
                bResult = true;
            }
            else
            {
                editor.SelectionStart = iStartOriginal;
                editor.SelectionEnd = iEndOriginal;
                bResult = false;
            }
        ...
        private void btnFindNext_Click(object sender, EventArgs e)
        {
            if (cboFind.Tag.ToString() == "")
            {
                cboFind.Tag = CountEditor(cboFind).ToString(); //btnFindNext_Click
            }
            
            FindNext4Editor(cboFind, btnFindNext.ToolTipText); //btnFindNext_Click
        }

        private void FindNext4Editor(C1ComboBox cboFind, string sFind, bool bMatchCase = false, bool bWholeWord = false)
        {
            if (cboFind.Text.Trim() == "")
            {
                cboFind.Text = "";
            ...
            UpdateFindList(cboFind); //btnFindNext_Click

            if (FindNext(cboFind, false, bMatchCase, bWholeWord) == false) //FindNext4Editor, 1st
            {
                //往下找不到資料，而且不是在第一格位置，再從頭開始找一次
                FindNext(cboFind, true, bMatchCase, bWholeWord); //FindNext4Editor, 2nd
            }
        }

        private bool FindNext(C1ComboBox cboFind, bool bFindAgain = false, bool bMatchCase = false, bool bWholeWord = false)
        {
            var sAllText = editor.Text;
            var sSearchText = bMatchCase == false ? cboFind.Text.ToUpper() : cboFind.Text;
            bool bResult;
            var bWholeWordStart = false;
            var bWholeWordEnd = false;
            var iStartOriginal = bFindAgain ? 0 : editor.SelectionStart;
            var iEndOriginal = bFindAgain ? 0 : editor.SelectionEnd;
            var iStart = 0;
            var iEnd = 0;
            var array = sAllText.ToCharArray();
            ...
            {
                SaveFindList("Editor", cboFind.Text); //FindNext
            }
            ...
            for (var i = iEndOriginal; i < array.Length; i++)
            {
                ...
                if (sLetter != sSearchText.Substring(0, 1)) continue; //第一個字母符合才要繼續
                ...
                for (var j = 1; j < sSearchText.Length; j++)
                {
                    ...
                    try
                    {
                        ...
                        if (j < sSearchText.Length && sLetter == sSearchText.Substring(j, 1))
                        {
                            bMatchFound = true;
                        }
                        else
                ...
                if (iStart == 0 || IsRealSymbol(editor.Text.Substring(iStart - 1, 1)))
                {
                    bWholeWordStart = true;
                }

                if (bWholeWordStart && (i + 1 == editor.TextLength || IsRealSymbol(editor.Text.Substring(i + 1, 1))))
                {
                    bWholeWordEnd = true;
                }
        ...
        {
            CountEditor(cboFind, true); //btnCount_Click
        }

        private void btnCountBox_Click(object sender, EventArgs e)
        {
            CountEditor(cboFindBox, true, (!btnMatchCase.Visible), (!btnMatchWholeWordOnly.Visible)); //btnCountBox_Click
        }

        private int CountEditor(C1ComboBox cboFind, bool bShowMsg = false, bool bMatchCase = false, bool bWholeWord = false)
        {
            var i = 0;
            var sText = editor.Text;
            var sFind = cboFind.Text;

            try
            {
                sText = GetPlainString(sText);
                sFind = GetPlainString(sFind);
                ...
                var matches = Regex.Matches(sText, sFind);
                ...
                foreach (Match m in matches)
                {
                    ...
                    else
                    {
                        ...
                        if (m.Index == 0 || IsRealSymbol(editor.Text.Substring(m.Index - 1, 1)))
                        {
                            bWholeWordStart = true;
                        }

                        if (bWholeWordStart && ((m.Index + sFind.Length) + 1 == editor.TextLength || IsRealSymbol(editor.Text.Substring(m.Index + sFind.Length, 1))))
                        {
                            bWholeWordEnd = true;
                        }

                        if (bWholeWordStart && bWholeWordEnd)
                        {
                            i++;
                        }
                    }
                ...
                if (bShowMsg)
                {
                    ...
                    var sTemp11 = MyGlobal.GetLanguageString("Match Case:", "form", Name, "msg", "MatchCase", "Text");
                    var sTemp12 = MyGlobal.GetLanguageString("Match Whole Word Only:", "form", Name, "msg", "MatchWholeWordOnly", "Text");
                    ...
                    MessageBox.Show(sTemp1 + @" " + cboFind.Text + "\r\n" + sTemp11 + " " + (bMatchCase ? "Yes" : "No") + "\r\n" + sTemp12 + @" " + (bWholeWord ? "Yes" : "No") + "\r\n\r\n" + sTemp2 + " " + i.ToString() + " " + sTemp3, @"JasonQuery", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(cboFind.Text + "\r\n\r\n" + ex.Message);
            }

            return i;
        }

        private string GetPlainString(string sText)
        {
            sText = sText.Replace("(", GetStringFromUnicode("FF8")); //（
            sText = sText.Replace(")", GetStringFromUnicode("FF9")); //）
            sText = sText.Replace("^", GetStringFromUnicode("FF3E")); //＾
            sText = sText.Replace("\\", GetStringFromUnicode("FF3C")); //＼
            sText = sText.Replace("*", GetStringFromUnicode("FFA")); //＊
            sText = sText.Replace("+", GetStringFromUnicode("FFB")); //＋
            sText = sText.Replace("-", GetStringFromUnicode("FFD")); //－
            sText = sText.Replace("|", GetStringFromUnicode("FF5C")); //｜
            sText = sText.Replace("?", GetStringFromUnicode("FF1F")); //？
        ...
        private void HighlightWord(C1ComboBox cboFind, string sText, bool bMatchCase = false, bool bWholeWord = false)
        {
            var sFindText = cboFind.Text;

            if (bMatchCase == false)
            {
                sText = sText.ToUpper();
                sFindText = sFindText.ToUpper();
            }

            if (string.IsNullOrEmpty(sText) || cboFind.Text.Trim() == "")
            {
                return;
            }
            ...
            UpdateHighlight(MyLibrary.sHighlightColorStyle, num);

            editor.Indicators[num].Under = true;
            editor.Indicators[num].ForeColor = ColorTranslator.FromHtml(MyLibrary.sHighlightColorForeColor);
            editor.Indicators[num].OutlineAlpha = Convert.ToInt16(MyLibrary.sHighlightColorOutlineAlpha);
            editor.Indicators[num].Alpha = Convert.ToInt16(MyLibrary.sHighlightColorAlpha);
            ...
            try
            {
                var matches = Regex.Matches(sText, sFindText);
                ...
                foreach (Match m in matches)
                {
                    if (bWholeWord == false)
                    {
                        ...
                        editor.IndicatorFillRange(m.Index, sFindText.Length);
                    }
                    else
                    {
                        var bWholeWordStart = false;
                        ...
                        if (bWholeWordStart && (m.Index + sFindText.Length + 1 == editor.TextLength || IsRealSymbol(editor.Text.Substring(m.Index + sFindText.Length, 1))))
                        {
                            bWholeWordEnd = true;
                        }
        ...
        private void OpenFile(bool bOpenFileOnCurrentTab, string sFilename = "")
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    while (MyGlobal.sCheckExistTabResult != "")
                    {
                        Application.DoEvents();
                        break;
                    }
        ...
        private bool SaveAs()
        {
            ...
            try
            {
                ...
                if (Path.GetExtension(sf.FileName) == "")
                {
                    sf.FileName = sf.FileName + ".sql";
                }
                ...
                case true when e.KeyCode == Keys.Y:
                    {
                        ...
                        editor.Redo();
                        CheckEditorContent(); //editor_KeyDown() 事件
                        break;
                    }
                default:
                    {
                        ...
                        if (c1GridAC4Period1.Visible)
                        {
                            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Space || e.KeyCode == Keys.Tab)
                            {
                                c1GridAC4Period1.Visible = false;
                            }
                            ...
                            //else if (c1GridAC4Period1.Focused && e.KeyCode == Keys.Back)
                            //{
                            //    if (editor.CurrentPosition <= _iPeriodPos)
                            //    {
                            //        c1GridAC4Period1.Visible = false;
                        ...
                        else if (c1GridAC4Space1.Visible)
                        {
                            ...
                            //else if (c1GridAC4Space1.Focused && e.KeyCode == Keys.Back)
                            //{
                            //    if (editor.CurrentPosition <= _iSpacePos)
                            //    {
                            //        c1GridAC4Space1.Visible = false;
        ...
        private DataTable DetectSchemaAndPeriod(string sAliasName)
        {
            var dt = new DataTable();
                ...
                case @"PostgreSQL":
                    {
                        ...
                        if (dtRow.Length > 0)
                        {
                            for (var i = 0; i < dtRow.Length; i++)
                            {
                                row = dt.NewRow();
                                row["SchemaName"] = dtRow[i]["SchemaName"];
                                row["SchemaType"] = dtRow[i]["SchemaType"];
                                dt.Rows.Add(row);
                            }
                ...
                case @"SQL Server":
                    {
                        if (sAliasName.IndexOf(".", StringComparison.Ordinal) == -1) //DB or Node
                        {
                            ...
                            if (dtRow.Length > 0) //DB
                            {
                                ...
                                for (var i = 0; i < dtRow.Length; i++)
                                {
                                    if (sDistinct.IndexOf("," + dtRow[i]["SchemaNode"] + ",", StringComparison.Ordinal) == -1)
                                    {
                                        sDistinct += dtRow[i]["SchemaNode"] + ",";

                                        row = dt.NewRow();
                                        row["SchemaName"] = dtRow[i]["SchemaNode"];
                                        row["SchemaType"] = "[" + dtRow[i]["DB"] + "]";
                                        dt.Rows.Add(row);
                                    }
                                }
                            ...
                            else //Node
                            {
                                ...
                                if (dtRow.Length > 0)
                                {
                                    for (var i = 0; i < dtRow.Length; i++)
                                    {
                                        row = dt.NewRow();
                                        row["SchemaName"] = dtRow[i]["SchemaName"];
                                        row["SchemaType"] = dtRow[i]["SchemaType"] + ", [" + dtRow[i]["DB"] + "]";
                                        dt.Rows.Add(row);
                                    }
        ...
        {
            if (editor.CurrentPosition < 4)
            {
                return; //長度不夠構成一個 Select 子句或是 Use 指令，直接忽略
            }

            var sSqlBlock = SelectBlock(false); //該段落的完整 SQL
            sSqlBlock = GetFormattedSql(sSqlBlock) + " "; //整理成一句 SQL

            if ((sSqlBlock.IndexOf("USE ", StringComparison.Ordinal) == -1) && (sSqlBlock.IndexOf(" FROM ", StringComparison.Ordinal) == -1))
            {
                return; //該段落 SQL 沒有找到 FROM 或 USE 關鍵字
            }
            ...
            for (var i = _iSpacePos - 1; i > 1; i--)
            {
                if ("` `\r`\n`".Contains("`" + editor.Text.Substring(i, 1) + "`"))
                {
                    continue; //游標所在處往前找
                }
                else
                {
                    //到此處，判斷是否為 FROM 或 USE 關鍵字
                    if ((editor.Text.Substring(i, 1).ToUpper() != "E") && (editor.Text.Substring(i, 1).ToUpper() != "M"))
                    {
                        _iSpacePos = 0;
                        break;
                    }
                    else if ((editor.Text.Substring(i - 2, 3).ToUpper() != "USE") && (editor.Text.Substring(i - 3, 4).ToUpper() != "FROM"))
                    {
                        _iSpacePos = 0;
                        break;
                    }
                    ...
                    {
                        bFrom = i > 3 && editor.Text.Substring(i - 3, 4).ToUpper() == "FROM";
                        bUse = editor.Text.Substring(i - 2, 3).ToUpper() == "USE";
            ...
            if (_iSpacePos == 0)
            {
                return; //按下空白處往前找，沒有找到 FROM；或是找到的不是 FROM；忽略！
            }

            var sCondition = "(SchemaType = '[Table]' OR SchemaType = '[View]')";
            ...
            if (MyGlobal.sDataSource == @"SQL Server")
            {
                ...
                else if (bFrom && !string.IsNullOrEmpty(MyGlobal.sDBConnectionDatabase))
                {
                    sCondition += " AND DB = '" + MyGlobal.sDBConnectionDatabase.ToUpper() + "'";
                }

                //var dt = new DataTable();
                //var dtRow = MyGlobal.dtTableAndViewName.Select();
                //dt.Columns.Add("SchemaName");
                //dt.Columns.Add("SchemaType");
                ...
                //        var row = dt.NewRow();
                //        row["SchemaName"] = dtRow[i]["SchemaName"];
                //        row["SchemaType"] = dtRow[i]["SchemaType"] + sDB;
                //        dt.Rows.Add(row);
                //    }
                ...
                //    dv.Sort = "SchemaName";
                //    dt = dv.ToTable();
                //    MyGlobal.dtTableAndViewName = dt;
                //}
            }
            else if (MyGlobal.sDataSource == @"MySQL")
            {
            ...
            //DataRow[] dtRow0 = MyGlobal.dtTableAndViewName.Select(sCondition);
            ////dt 驗證用！
            //var dt0 = dtRow0[0].Table.Clone(); //複製 DataRow 的表結構

            //foreach (DataRow row in dtRow)
            //{
            //    dt0.Rows.Add(row.ItemArray); //將 DataRow 新增到 DataTable 中
            //}
            ...
            if (dtRow.Length > 0)
            {
                if (bUse)
                {
                    ...
                    for (var i = 0; i < dtRow.Length; i++)
                    {
                        if (sDistinct.IndexOf("`" + dtRow[i]["DB"] + "`", StringComparison.Ordinal) == -1)
                        {
                            sDistinct += dtRow[i]["DB"] + "`";

                            var row = dt.NewRow();
                            row["SchemaName"] = dtRow[i]["DB"];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                var iTop = p.Y - MyGlobal.iMainFormTop - 30;
                ...
                ResizeACGrid(c1GridAC4Space1, c1GridAC4Space2, _dtAC4Space.Rows.Count, iWidth);
            }
        }
        ...
        /// <returns></returns>
        private string GetACSql4Subquery(string sSql, int iStart)
        {
            var iEnd = 0;
            var bDoubleQuotation = false; //是否為雙引號起始？
            var bSingleQuotation = false; //是否為單引號起始？
        ...
        /// <param name="iStart">從哪一個位置開始往後搜尋</param>
        /// <returns></returns>
        private string GetACSql4WithAs(string sSql, int iStart)
        {
            var iEnd = 0;
            var bDoubleQuotation = false; //是否為雙引號起始？
            var bSingleQuotation = false; //是否為單引號起始？
        ...
        private void HandleAC4Period(int iPosNew = 0) //iPosNew: 透過 Ctrl+J 傳過來的變數
        {
            ...
            var sAliasName = "";
            var iBlankPos = 0; //記住找到 Alias Name 的空白處
            ...
            if (iPos == -1)
            {
                //20220728 這裡要考慮它是不是已是結尾或是換行符號
                iPos = sSqlBlock.ToUpper().IndexOf(" " + sAliasName + " ", StringComparison.Ordinal);
            }
            ...
            else
            {
                ...
                {
                    var j = sSqlBlock.IndexOf(" " + sAliasName + " ", StringComparison.Ordinal) - 1;
            ...
            if (bShow)
            {
                ...
                ResizeACGrid(c1GridAC4Period1, c1GridAC4Period2, _dtAC4Period.Rows.Count, iWidth);
            }
        }
        ...
        private void editor_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            //AutoComplete (偵測鍵盤英文字母區塊，逗號旁邊的句點 Keys.OemPeriod；或是數字九宮格區塊的句點 Keys.Decimal)
            if (1 == 1)
            {
                //重點：
                ...
                if (e.KeyData == Keys.Space)
                {
                    if (c1GridAC4Period1.Visible)
                    {
                        c1GridAC4Space1.Visible = false; //Table name or View name 不應該有 空白，故輸入 空白 則要隱藏下拉清單
                    }
                    else if (c1GridAC4Period1.Visible == false)
                    {
                ...
                else if (e.KeyData == Keys.OemPeriod || e.KeyData == Keys.Decimal)
                {
                    ...
                    else if (c1GridAC4Space1.Visible == false)
                    {
                ...
                else
                {
                    if (_bKeyCodeCtrlJ)
                    {
                        _bKeyCodeCtrlJ = false; //Ctrl+J 會回到這裡，不重複處理！
                    }
                    else if (c1GridAC4Period1.Visible) //句點觸發
                    {
                        if (e.KeyCode == Keys.Home || e.KeyCode == Keys.PageUp || e.KeyCode == Keys.PageDown)
                        {
                            c1GridAC4Period1.Visible = false;
                        }
                        else if (e.KeyCode == Keys.End)
                        {
                            if (editor.CurrentPosition < _iPeriodPos - 1)
                            {
                                c1GridAC4Period1.Visible = false; //還落在句點上面，顯示！超出句點才要隱藏
                            }
                            else
                            {
                                var bHide = true;
                        ...
                        else if (editor.CurrentPosition <= _iPeriodPos - 1) //可能是按下倒退鍵或左鍵
                        {
                        ...
                        else
                        {
                            ...
                            var iWidth = MyGlobal.ResizeGridColumnWidth(c1GridAC4Period1);
                            ResizeACGrid(c1GridAC4Period1, c1GridAC4Period2, iRowCount, iWidth);
                        }
                    }
                    else if (c1GridAC4Space1.Visible) //空白觸發
                    {
                        ...
                        else if (e.KeyCode == Keys.End)
                        {
                            if (editor.CurrentPosition < _iSpacePos - 1)
                            {
                                c1GridAC4Space1.Visible = false; //還落在空白上面，顯示！超出空白才要隱藏
                            }
                            else
                            {
                                var bHide = true;
                        ...
                        else if (editor.CurrentPosition <= _iSpacePos - 1) //可能是按下倒退鍵或左鍵
                        {
                        ...
                        else
                        {
                            ...
                            var iWidth = MyGlobal.ResizeGridColumnWidth(c1GridAC4Space1);
                            ResizeACGrid(c1GridAC4Space1, c1GridAC4Space2, iRowCount, iWidth);
                        }
                    }
                }
            }
        ...
        private void ResizeACGrid(C1TrueDBGrid c1Grid1, C1TrueDBGrid c1Grid2, int RowCount, int iWidth)
        {
            var iHeight = 0;
            var iWidth2 = 0;
            ...
            {
                iHeight = 181;
                iWidth = MyGlobal.ResizeGridColumnWidth(c1Grid2);

                for (var i = 1; i < 50; i++)
                {
                    c1Grid2.Size = new Size(iWidth, iHeight);

                    if (c1Grid2.HScrollBar.Visible == false)
                    {
                        break;
                    }
                    else
                ...
                iWidth2 = iWidth;
                iWidth = MyGlobal.ResizeGridColumnWidth(c1Grid2) + 50;
                iHeight = 20;

                for (var i = 20; i < 200; i++)
                {
                    c1Grid2.Size = new Size(iWidth, i);

                    if (c1Grid2.VScrollBar.Visible == false)
                    {
                        break;
                    }
                    else
                ...
                c1Grid1.Size = new Size(iWidth2, iHeight);
                c1Grid2.Size = new Size(iWidth2, iHeight);

                if (c1Grid1.HScrollBar.Visible || c1Grid1.VScrollBar.Visible)
                {
                    c1Grid1.Size = new Size(iWidth2, iHeight + c1Grid1.VScrollBar.Width);
                    //c1Grid1.Size = new Size(iWidth2 + 2, iHeight - c1Grid1.VScrollBar.Width + 2);
                }
            }
            else
            {
        ...
        private void txtIndentWord_Leave(object sender, EventArgs e)
        {
            ...
            if (txtIndentWord.Text == "" || txtIndentWord.Text == @"0")
            {
        ...
        private void timerMother2Child_Tick(object sender, EventArgs e)
        {
            ...
            if (MyGlobal.sCheckFileFromMDIForm != "" && !string.IsNullOrEmpty(AccessibleDescription) && (MyGlobal.sCheckFileFromMDIForm.Contains("`" + AccessibleDescription + "`")))
            {
            ...
            //判斷是否為指定的子表單
            if (MyGlobal.sOpenFileFromMDIForm != "" && !string.IsNullOrEmpty(AccessibleDescription) && MyGlobal.sOpenFileFromMDIForm.Substring(0, 17) == AccessibleDescription)
            {
            ...
            //SingleRecordViewer表單傳來的要求？
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("singlerecordviewer" + MyGlobal.sSeparator))
            {
                ...
                {
                    int.TryParse(sTempRow, out int iCurrentRow);

                    var c1Grid = GetWhichGrid();
                    var dtData = (DataTable)c1Grid.DataSource;
                    int iTotalRows = dtData.Rows.Count;
            ...
            //是否為 Editor Setting 套用？
            if (MyGlobal.sInfoFromMDIForm != "" && MyGlobal.sInfoFromMDIForm.StartsWith("reloadqueryeditorsetting`"))
            {
                ...
                if (sTemp == AccessibleDescription) //確認是否為指定的 Tab
                {
                    ...
                    if (MyLibrary.bEnableAutoComplete) //20220523
                    {
            ...
            //是否為「按下 Commit / Rollback 按鈕」，或「執行 commit / rollback 指令」？
            if (MyGlobal.sInfoFromMDIForm != "" && MyGlobal.sInfoFromMDIForm.StartsWith("executecommitrollback`"))
            {
            ...
            //如果是 nonquery, 執行無錯誤, 更新 Commit / Rollback 按鈕
            if (MyGlobal.sInfoFromMDIForm != "" && MyGlobal.sInfoFromMDIForm.StartsWith("updatecommitrollbackbutton`"))
            {
            ...
            //如果是單純查詢，且不需要等待 Commit / Rollback，全部「中斷連線」
            if (MyGlobal.sInfoFromMDIForm != "" && MyGlobal.sInfoFromMDIForm.StartsWith("disconnectafterqueryonly`"))
            {
            ...
            //如果是 PostgreSQL，執行指令有錯誤：全部中斷連線
            if (MyGlobal.sInfoFromMDIForm != "" && MyGlobal.sInfoFromMDIForm.StartsWith("disconnectafterexecuteerror`"))
            {
            ...
            //變數表單回傳的 SQL？
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("parameters" + MyGlobal.sSeparator))
            {
            ...
            //是否為主表單通知要自動中斷連線？
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("autodisconnect`"))
            {
            ...
            //是否為主表單詢問 Query 功能表 《Enable/Disable》？
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("mainforminfo@menu2`"))
            {
                ...
                if (sTemp == AccessibleDescription) //確認是否為指定的 Tab
                {
                    ...
                    MyGlobal.sGlobalTemp = "childforminfo@menu2`" + (editor.CanUndo == false ? "0" : "1") + ";" + (editor.CanRedo == false ? "0" : "1") + ";" + (editor.SelectedText == "" ? "0" : "1") + ";" + (editor.CanPaste ? "1" : "0") + ";" + (editor.Text == "" ? "0" : "1");
                }
            }

            //是否為主表單詢問 Edit 功能表 《Enable/Disable》？
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("mainforminfo@menu1`"))
            {
                ...
                if (sTemp == AccessibleDescription) //確認是否為指定的 Tab
                {
                    ...
                    //判斷是否有選取文字，決定功能表項目可不可用
                    MyGlobal.sGlobalTemp = "childforminfo@menu1`" + (editor.CanUndo == false ? "0" : "1") + ";" + (editor.CanRedo == false ? "0" : "1") + ";" + (editor.SelectedText == "" ? "0" : "1") + ";" + (editor.CanPaste ? "1" : "0") + ";" + (editor.Text == "" ? "0" : "1");
                }
            }

            //是否為主表單傳來的 Edit 功能表 《指令》？
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("mainformaction@menu1`"))
            {
            ...
            //是否為主表單傳來的 File 功能表 指令？
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("mainformaction@menu0`"))
            {
            ...
            if (MyGlobal.sInfoFromReloadLocalization != "" && MyGlobal.sInfoFromReloadLocalization.StartsWith("reloadlocalization`"))
            {
            ...
            //執行 SQL Statement 出錯了：取得波浪底線要呈現的位置
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("sqlexecuteerrorpos" + MyGlobal.sSeparator))
            {
            ...
            //產生 SQL 指令：貼至 Query Editor
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("generatesqlpaste2queryeditor" + MyGlobal.sSeparator))
            {
            ...
            //20220423 SQL Server 使用 USE 指令切換資料庫，重新連線！
            if (MyGlobal.sGlobalTemp2 != "" && MyGlobal.sDataSource == @"SQL Server" && MyGlobal.sGlobalTemp2.StartsWith("sqlserverswitchdatabase" + MyGlobal.sSeparator))
            {
                ...
                if (sTemp2 == AccessibleDescription) //確認是否為指定的 Tab
                {
                    ...
                    //20220423 不能重新連線，否則 Connection 會跟之前的不同，導致之前未 Commit 的異動會失效！
            ...
            //20220515 MySQL 使用 USE 指令切換資料庫，重新連線！
            if (MyGlobal.sGlobalTemp2 != "" && MyGlobal.sDataSource == @"MySQL" && MyGlobal.sGlobalTemp2.StartsWith("mysqlswitchdatabase" + MyGlobal.sSeparator))
            {
            ...
            //執行「異動的 SQL」，取得異動的筆數
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("sqlexecuteaffected" + MyGlobal.sSeparator))
            {
                ...
                if (sTemp2 == AccessibleDescription) //確認是否為指定的 Tab
                {
                    ...
                    txtMessage.Tag = "nonquery";
                    txtMessage.LineScroll(txtMessage.Lines.Count, 0);
                }
            }

            //檢查是否需要 commit
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("closequeryformandcheckcommit`"))
            {
                ...
                if (sTemp == AccessibleDescription) //確認是否為指定的 Tab
                {
                    ...
                    if (lblNotCommitYet.Text != "" && MyGlobal.iCommitCheck == -1)
                    {
                    ...
                    if (MyGlobal.sGlobalTemp == "closequeryformandcheckcommit`") { MyGlobal.sGlobalTemp = ""; }
                }
            }

            //由 TabControl 關閉 Query Form (關閉單一 Form)
            if (MyGlobal.sGlobalTemp != "" && MyGlobal.sGlobalTemp.StartsWith("closequeryform`"))
            {
            ...
            if (!string.IsNullOrEmpty(AccessibleDefaultActionDescription) && AccessibleDefaultActionDescription.Length > 0) // && (AccessibleDescription.Length - AccessibleDescription.Replace("`", "").Length == 1))
            {
                ...
                else if (AccessibleDefaultActionDescription.StartsWith("OPEN0:") || AccessibleDefaultActionDescription.StartsWith("OPEN1:") || AccessibleDefaultActionDescription.StartsWith("OPEN2:"))
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (MyGlobal.sCancelOpenAndCloseTab != "")
                            {
            ...
            //「不是」第一次開啟 Query Form，要透過全域變數來判斷
            if (MyGlobal.sInfoFromMDIForm != "" && MyGlobal.sInfoFromMDIForm.StartsWith("transferselectsql`"))
            {
                ...
                if (sTemp.Split(';')[0] == AccessibleDescription) //確認是否為指定的 Tab
                {
                    ...
                    var i = editor.Text.Length + (editor.Text == "" ? 0 : 2);
                    editor.Text = (editor.Text == "" ? sTemp : editor.Text + "\r\n" + sTemp);
            ...
            //判斷是否有從外部傳進來要開啟的檔案
            if (MyGlobal.sOpenFileFromExternal != "")
            {
            ...
            switch (txtMessage.Tag.ToString())
            {
                case @"error":
                    {
                        sResult = "Error";
                ...
                //非查詢模式，且沒有發生錯誤
                case @"nonquery":
                    editorSQLHistory.Text = editorSQLHistory.Text.Replace(MyGlobal.sSeparator3, "\r\n--Status: \"" + _sQueryStatus + "\", " + lblExecTime.Text + ", " + txtMessage.Text.Replace("\r\n", "\r\n--"));
                    sMessage = txtMessage.Text;
                    break;
                default:
            ...
            if (MyGlobal.sExecuteNonQuerySQLHistoryScript != "")
            {
            ...
            else
            {
                ...
                {
                    sNewMessage = sMessage == "" ? _sQueryTextParametersMapping.Replace("'", "''") : sMessage.Replace("'", "''") + (_sQueryTextParametersMapping == "" ? "" : "\r\n\r\n" + _sQueryTextParametersMapping.Replace("'", "''"));
                }
            ...
            btnAutoSort.Enabled = chkSort.Checked != true && bValue;

            //傳訊息至母表單
            //uTransferValueToMainForm("p`Processing OK!"); //Total Records: " + c1TrueDBGrid1.Splits[_SplitsNum].Rows.Count.ToString());

            if (txtMessage.Tag.ToString() == @"nonquery" || txtMessage.Tag.ToString() == @"cancel")
            {
                c1DockingTab1.SelectedTab = tabMessage;
            }
            ...
            else if (txtMessage.Tag.ToString() != @"error")
            {
            ...
            if (txtMessage.Tag.ToString() == "" || txtMessage.Tag.ToString() != @"error")
            {
            ...
            else if (txtMessage.Tag.ToString() == @"error" && MyGlobal.sDataSource == @"PostgreSQL") //&& MyGlobal.bDBAutoRollback)
            {
        ...
        private void UpdateSqlHistory(int iRows, string sResult, string sMessage, string sSQL)
        {
            ...
            MDBCommon.ExecNonQuery("Insert Into SQLHistory (MPID, ExecutionDate, ExecutionTime, QueryTime, Rows, Result, Message, SQL) Values (" + MyGlobal.sDBMotherPID + ", '" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "', '" + sExecTime + "', '" + sQueryTime + "', " + iRows + ", '" + sResult + "', '" + sMessage + "', '" + sSQL + "')");
        }
        ...
        private static void LoadFindList(string sFun, C1ComboBox objComboBox)
        {
            ...
            try
            {
                var dtRecent = MDBCommon.ExecQuery("Select AttributeValue From SystemConfig Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='FindList_" + sFun + "' Order By AttributeDate Desc");
        ...
        {
            //Save
            var dtRecent = MDBCommon.ExecQuery("Select * From SystemConfig Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='FindList_" + sFun + "' And AttributeValue='" + sFindText.Replace("'", "''") + "'");

            if (dtRecent.Rows.Count > 0)
            {
                MDBCommon.ExecNonQuery("Update SystemConfig Set AttributeDate='" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "' Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='FindList_" + sFun + "' And AttributeValue='" + sFindText.Replace("'", "''") + "'");
            }
            else
            {
                MDBCommon.ExecNonQuery("Insert Into SystemConfig (DomainUser, MPID, AttributeKey, AttributeValue, AttributeDate) Values ('" + MyGlobal.sDomainUser + "', " + MyGlobal.sDBMotherPID + ", 'FindList_" + sFun + "', '" + sFindText.Replace("'", "''") + "', '" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "')");
            }
        ...
        private void LoadReplaceList()
        {
            ...
            try
            {
                var dtReplace = MDBCommon.ExecQuery("Select AttributeValue From SystemConfig Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='ReplaceList_Editor" + "' Order By AttributeDate Desc");
        ...
        {
            //Save
            var dtRecent = MDBCommon.ExecQuery("Select * From SystemConfig Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='ReplaceList_" + sFun + "' And AttributeValue='" + sReplaceText.Replace("'", "''") + "'");

            if (dtRecent.Rows.Count > 0)
            {
                MDBCommon.ExecNonQuery("Update SystemConfig Set AttributeDate='" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "' Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='ReplaceList_" + sFun + "' And AttributeValue='" + sReplaceText.Replace("'", "''") + "'");
            }
            else
            {
                MDBCommon.ExecNonQuery("Insert Into SystemConfig (DomainUser, MPID, AttributeKey, AttributeValue, AttributeDate) Values ('" + MyGlobal.sDomainUser + "', " + MyGlobal.sDBMotherPID + ", 'ReplaceList_" + sFun + "', '" + sReplaceText.Replace("'", "''") + "', '" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "')");
            }
        ...
        private void LoadSplitterData(string sSplitterKey)
        {
            try
            {
                var dtSplitterData = MDBCommon.ExecQuery("Select AttributeValue From SystemConfig Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='SplitterConfig' And AttributeName='QueryEditor_" + sSplitterKey + "'");
        ...
        private void SaveSplitterData(string sSplitterKey, int iSplitterValue) //sSplitterKey="U/D" or "L/R"
        {
            ...
            //Save
            var dtSplitterData = MDBCommon.ExecQuery("Select * From SystemConfig Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='SplitterConfig' And AttributeName='QueryEditor_" + sSplitterKey + "'");

            if (dtSplitterData.Rows.Count > 0)
            {
                MDBCommon.ExecNonQuery("Update SystemConfig Set AttributeValue='" + iSplitterValue + "' Where DomainUser='" + MyGlobal.sDomainUser + "' And MPID=" + MyGlobal.sDBMotherPID + " And AttributeKey='SplitterConfig' And AttributeName='QueryEditor_" + sSplitterKey + "'");
            }
            else
            {
                MDBCommon.ExecNonQuery("Insert Into SystemConfig (DomainUser, MPID, AttributeKey, AttributeName, AttributeValue) Values ('" + MyGlobal.sDomainUser + "', " + MyGlobal.sDBMotherPID + ", 'SplitterConfig', 'QueryEditor_" + sSplitterKey + "', '" + iSplitterValue + "')");
            }
        }
        ...
        private void Indent(int iShift = 1) //加上 -- 註解
        {
            ...
            for (var i = 0; i < parts.Length; i++)
            {
                ...
                if (parts[i] != "") //如果該列是完全空的，就不在最前面加上空白了！
                {
        ...
        private string SelectBlock(bool bSelect = true)
        {
            ...
            return editor.GetTextRange(iStart2, iEnd2 - iStart2);
        ...
        private void GetNewStartAndEnd_EntireBlankRowAsEmptyRow(int iStart, int iEnd, ref int iStart2, ref int iEnd2)
        {
            ...
            for (var i = iCurrentLine; i > 0; i--)
            {
                ...
                if (sLine.Trim() != "" || i == iCurrentLine) continue; //如果整列都是空白
            ...
            else
            {
                for (var i = iCurrentLine; i < parts.Length; i++)
                {
                    ...
                    if (sLine.Trim() == "" && i != iCurrentLine) //如果整列都是空白
                    {
                        iLineEnd = i - 1;
                        break;
                    }

                    if (sLine.Trim() == "" && i == iCurrentLine)
                    {
                        iLineEnd = i;
                        break;
                    }
        ...
        private void UpdateEditorStatusBarLanguage()
        {
            ...
            lblEditorLines.Text = _sEditorLines + @" " + lines.Length;
            lblEditorLength.Text = _sEditorLength + @" " + (editor.TextLength);
            lblEditorLn.Text = _sEditorLn + @" " + (editor.CurrentLine + 1);
            lblEditorCol.Text = _sEditorCol + @" " + (editor.GetColumn(editor.CurrentPosition) + 1);
            lblEditorPos.Text = _sEditorPos + @" " + (editor.CurrentPosition + 1);
            lblEditorSel.Text = _sEditorSel + @" " + iSel;
        ...
        private void ApplySqlStyler()
        {
            ...
            txtMessage.Styler = new SqlStyler(); //變更關鍵字、顏色
        }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            switch (keyData)
            {
                case Keys.F12:
                    ...
                    break;
                case Keys.F3:
                    if (editor.Focused || c1TrueDBGrid1.Focused)
                    {
                        if (editor.Focused)
                        {
                            btnFindNext.PerformClick();
                ...
                case Keys.Control | Keys.C: //Ctrl+C
                    ...
                    else if (MyLibrary.bCopyAsHTML && editor.Focused && editor.Tag.ToString() == "")
                    {
                        try
                        {
                            Clipboard.Clear();
                ...
                case Keys.Control | Keys.J: //Ctrl+J
                    ...
                    if (1 == 1 && editor.Focused && c1GridAC4Period1.Visible == false)
                    {
                        var iTemp = 0;
                        var iPeriodPos = 0;
                        ...
                        {
                            for (var i = editor.CurrentPosition; i > 18; i--)
                            {
                                if (eText.Substring(i, 1) == " " && eText.Substring(i - 1, 1) == " ")
                                {
                ...
                case Keys.Control | Keys.H: //Ctrl+H 取代
                    ShowFindAndReplace(keyData == (Keys.Control | Keys.F) ? "F" : "R"); //ProcessCmdKey
                    break;
                ...
                case Keys.Escape:
                    ...
                    else if (pnlFindReplace.Visible)
                    {
        ...
        private void ShowFindAndReplace(string sMode)
        {
            tsClose.BackColor = SystemColors.Control;
            tsMatch.BackColor = SystemColors.Control;
            ...
            {
                case "F":
                    cboFindBox.Text = sWord;
                    btnFindNextBox.Enabled = (sWord != "");
                    btnFindPreviousBox.Enabled = (sWord != "");
                    btnHighlightAllBox.Enabled = (sWord != "");
                    btnCountBox.Enabled = (sWord != "");
                    cboFindBox.Focus();
                    break;
                case "R":
                    cboFindBox.Text = sWord;
            ...
            tsClose.BackColor = pnlFindReplace.BackColor;
            tsHighlight.BackColor = pnlFindReplace.BackColor;
            tsMatch.BackColor = pnlFindReplace.BackColor;
        }
        ...
        private void HighlightSelection(bool bMouseClick = false)
        {
            ...
            if (editor.SelectedText == "" && bMouseClick)
            {
                editor.Tag = "";
            }

            if (word == "" || !bValue || editor.Tag.ToString() == word)
            {
                return;
            }
        ...
        private void btnHighlightAll_Click(object sender, EventArgs e)
        {
            ...
            if (tsb.Name == "btnHighlightAll")
            {
                btnClearHighlights.PerformClick();
                UpdateFindList(cboFind); //btnHighlightAll_Click

                if ((cboFind.Tag ?? string.Empty).ToString() == "")
                {
                    cboFind.Tag = CountEditor(cboFind, false, !btnMatchCase.Visible).ToString(); //btnFindPrevious_Click
                }

                HighlightWord(cboFind, editor.Text); //btnHighlightAll_Click, "btnHighlightAll"
            }
            else
            {
                btnClearHighlightsBox.PerformClick();
                UpdateFindList(cboFindBox); //btnHighlightAll_Click

                if ((cboFindBox.Tag ?? string.Empty).ToString() == "")
                {
                    cboFindBox.Tag = CountEditor(cboFindBox, false, !btnMatchCase.Visible).ToString(); //btnFindPrevious_Click
                }

                HighlightWord(cboFindBox, editor.Text, btnMatchCase.Visible != true, btnMatchWholeWordOnly.Visible != true); //btnHighlightAll_Click, "btnHighlightAllBox"
            }
        }
        ...
        {
            var bValue = editor.SelectedText.Trim() != "";
        ...
        {
            var bValue = cboFindGrid.Text.Trim() != "";
        ...
        {
            if (e.KeyChar != 13 || cboFindGrid.Text.Trim() == "")
            {
                return;
            }
        ...
        private void cboFindGrid_TextChanged(object sender, EventArgs e)
        {
            ...
            //cboFindGrid.Tag = iCount.ToString();
                ...
                case MouseButtons.Right:
                    {
                        ...
                        var sCells = GetGridSelectedXY(iCol, iRow);
                        ...
                        var bValueUnfreeze = _UnfreezeColumnName != "";
                        ...
                        _sLangText = MyGlobal.GetLanguageString("Unfreeze Column", "form", Name, "menugrid", "UnfreezeColumn", "Text");
                        _cMenuGrid.Items[(int)_eMenuGrid.UnfreezeColumn].Text = _sLangText + (_UnfreezeColumnName == "" ? "" : " (" + _UnfreezeColumnName + ")");
        ...
        private void FrozenColumn(bool bFrozen = true)
        {
            ...
            _UnfreezeColumnName = c1Grid.Splits[_SplitsNum].DisplayColumns[c1Grid.Col].ToString();
        ...
        private void ArrangeData(string sMode)
        {
            ...
            bool bSelectedWholeColumn = c1Grid.SelectedRows.Count == 0 && c1Grid.SelectedCols.Count > 0;
                ...
                for (var row = 0; row < c1Grid.Splits[_SplitsNum].Rows.Count; row++)
                {
                    foreach (var col in c1Grid.SelectedCols)
                    {
                        ...
                        if (row == 0)
                        {
                            ...
                            sDataType += (sTemp2 == "") ? "" : sTemp2 + sFieldSeparator;
                        }
                    ...
                    if (bActiveCell)
                    {
                        ...
                        foreach (C1DataColumn col1 in c1Grid.Columns)
                        {
                            if (i == c1Grid.Col)
                            {
                                ...
                                if (sColumnDataType.ToUpper() == "STRING" || sColumnDataType.ToUpper() == "DATETIME")
                                {
                                    sData = sQuotingWith + c1Grid[c1Grid.Splits[_SplitsNum].Rows[c1Grid.Row].DataRowIndex, c1Grid.Col] + sQuotingWith;
                                }
                                else if (sColumnDataType.ToUpper() == "NSTRING") //20220711 這個欄位要特別處理！
                                {
        ...
        private void ArrangeData4AllData(string sMode, bool bToDate = false)
        {
            ...
            else
            {
                ...
                switch (sMode)
                {
                    case "ExportAllDataToCSV":
                        ...
                        _sLangText = _sLangText + " - " + sTitleName;
                        sf.FileName = MyLibrary.sGridSheetName;
                        break;
            ...
            Cursor = Cursors.WaitCursor;
            selCol = c1Grid.SelectedCols.Count;
            var sQuotingWith = "";
            ...
            if (chkShowColumnType.Checked && sDataType != "")
            {
                sDataType = sDataType.Substring(0, sDataType.Length - sFieldSeparator.Length);
            }
        ...
        private void SetSquiggles(string sSqlExecuted, string sPos, string sErrMsg, out int iCursorPostion, out int iLength)
        {
            ...
            foreach (var t in parts2)
            {
                ...
                int.TryParse(sLine, out int iErrLine);
                int.TryParse(sPosition, out int iPosition);
        ...
        private void SingleRecordViewer()
        {
            ...
            dtTemp = MDBCommon.ExecQuery(sSql);
            ...
            using (var myForm = new frmSingleRecordViewer())
            {
                ...
                myForm.sTitleText = "SingleRecordViewer";
                myForm.ShowDialog();
            }
        }

        private void UpdateFindList(C1ComboBox cboFind) //判斷是否要更新「搜尋清單」
        {
            if (lblFind.Tag.ToString() == cboFind.Text)
            {
                return;
            }
            ...
            lblFind.Tag = cboFind.Text;
        }

        private void UpdateReplaceList() //判斷是否要更新「搜尋清單」
        {
            if (cboReplaceBox.Tag.ToString() == cboReplaceBox.Text)
            {
                return;
            }

            if (cboReplaceBox.Items.Count > 0 && cboReplaceBox.Text == cboReplaceBox.Items[0].ToString())
            {
                //搜尋的字串是第一個，不用更新
            }
            else
            {
                SaveReplaceList("Editor", cboReplaceBox.Text); //UpdateFindList
            }

            cboReplaceBox.Tag = cboFind.Text;
        }
        ...
        private void btnAutoSort_Click(object sender, EventArgs e)
        {
            ...
            var sArrayHeader = sHeader.Split(sArraySeparators, StringSplitOptions.RemoveEmptyEntries);
            //Array.Sort(sArrayHeader, string.CompareOrdinal); //ASCII排序, 20220514 改用 LINQ 寫法，避免字母大小寫被分別排序
        ...
        private void c1TrueDBGrid1_Enter(object sender, EventArgs e)
        {
            ChangeBackColor(_cToolstripFocused);
        }

        private void c1TrueDBGrid1_Leave(object sender, EventArgs e)
        {
            ChangeBackColor(_cToolstripUnfocused);
        }
        ...
        private void editorSQLHistory_Leave(object sender, EventArgs e)
        {
            ChangeBackColor(_cToolstripUnfocused);
        }
        ...
        {
            UpdateCursor(); //txtMessage_MouseMove
        }
        ...
        private void UpdateAutoReplaceDictionary()
        {
            ...
            for (var i = 0; i < c1GridARInfo.Splits[_iii].Rows.Count; i++)
            {
                ...
                if (sKeyword.Trim() != "" && sReplacement.Trim() != "")
                {
        ...
        private void btnFindNextBox_Click(object sender, EventArgs e)
        {
            UpdateFindList(cboFindBox); //btnFindNextBox_Click
            cboFind.Text = cboFindBox.Text;
            cboFindBox.Tag = CountEditor(cboFindBox, false, (btnMatchCase.Visible != true), (btnMatchWholeWordOnly.Visible != true)).ToString(); //btnFindNextBox_Click
            FindNext4Editor(cboFindBox, btnFindNextBox.Text, (btnMatchCase.Visible != true), (btnMatchWholeWordOnly.Visible != true)); //btnFindNextBox_Click
        }

        private void btnFindPreviousBox_Click(object sender, EventArgs e)
        {
            UpdateFindList(cboFindBox); //btnFindPreviousBox_Click
            cboFind.Text = cboFindBox.Text;
            cboFindBox.Tag = CountEditor(cboFindBox, false, (btnMatchCase.Visible != true), btnMatchWholeWordOnly.Visible != true).ToString(); //btnFindPreviousBox_Click
            FindPrevious4Editor(cboFindBox, btnFindPreviousBox.Text, btnMatchCase.Visible != true, btnMatchWholeWordOnly.Visible != true);
        }

        private void btnReplace_Click(object sender, EventArgs e)
        {
            var sReplaceText = MyGlobal.GetLanguageString("Replace", "form", Name, "msg", "ReplaceText", "Text");
            cboFindBox.Tag = CountEditor(cboFindBox, false, !btnMatchCase.Visible, btnMatchWholeWordOnly != null && !btnMatchWholeWordOnly.Visible).ToString(); //btnReplace_Click, 取代前的計算

            if (cboFindBox.Tag.ToString() == "0")
            {
                _sLangText = MyGlobal.GetLanguageString("{qty} occurrence(s) were replaced.", "form", Name, "msg", "Replaced", "Text");
                SetLabelInfoEditor(sReplaceText + ": " + _sLangText.Replace("{qty}", "0"), Color.Red);
            }
            else
            {
                UpdateFindList(cboFindBox); //btnReplace_Click
                UpdateReplaceList(); //btnReplace_Click

                if (!string.Equals(editor.SelectedText, cboFindBox.Text, StringComparison.CurrentCultureIgnoreCase)) //沒有選取到「尋找的關鍵字」
                {
                    btnFindNextBox.PerformClick();
                }
                else //執行「取代」動作
                {
                    editor.ReplaceSelection(cboReplaceBox.Text);
                    cboFindBox.Tag = CountEditor(cboFindBox, false, (!btnMatchCase.Visible), (btnMatchWholeWordOnly.Visible != true)).ToString(); //btnReplace_Click, 取代後，再計算一次

                    if (cboFindBox.Tag.ToString() == "0")
                    {
                        _sLangText = MyGlobal.GetLanguageString("1 occurrence was replaced. The next occurrence not found.", "form", Name, "msg", "ReplaceOneAndNextNotFound", "Text");
                        SetLabelInfoEditor(sReplaceText + ": " + _sLangText, Color.Black);
                    }
                    else
                    {
                        _sLangText = MyGlobal.GetLanguageString("1 occurrence was replaced. The next occurrence found.", "form", Name, "msg", "ReplaceOneAndNextFound", "Text");
                        SetLabelInfoEditor(sReplaceText + ": " + _sLangText, Color.Black);
                        btnFindNextBox.PerformClick();
                    }
                }
        ...
        {
            var iShift = 0;
            var iCurrentPos = editor.SelectionStart;
            var sText = GetPlainString(editor.Text);
            var sFind = GetPlainString(cboFindBox.Text);
            var iCount = CountEditor(cboFindBox, false, btnMatchCase.Visible != true, btnMatchWholeWordOnly.Visible != true); //btnReplaceAll_Click
            cboFindBox.Tag = iCount.ToString();
            var sReplaceAllText = MyGlobal.GetLanguageString("Replace All", "form", Name, "msg", "ReplaceAllText", "Text");
            _sLangText = MyGlobal.GetLanguageString("{qty} occurrence(s) were replaced.", "form", Name, "msg", "Replaced", "Text");
            var iCountReplaced = 0;

            Cursor = Cursors.WaitCursor;

            if (cboFindBox.Tag.ToString() == "0")
            {
                SetLabelInfoEditor(sReplaceAllText + ": " + _sLangText.Replace("{qty}", "0"), Color.Red);
            }
            else
            {
                UpdateFindList(cboFindBox); //btnReplaceAll_Click
                UpdateReplaceList(); //btnReplaceAll_Click

                editor.SelectionStart = 0;

                if (btnMatchCase.Visible)
                {
                    sText = sText.ToUpper();
                    sFind = sFind.ToUpper();
                }
                ...
                foreach (Match m in matches)
                {
                    ...
                    if (btnMatchWholeWordOnly.Visible)
                    {
                        bReplace = true;
                    }
                    else
                    {
                        if (m.Index + iShift == 0 || IsRealSymbol(editor.Text.Substring(m.Index + iShift - 1, 1)))
                        {
                            bWholeWordStart = true;
                        }

                        if (bWholeWordStart && (m.Index + iShift + sFind.Length + 1 == editor.TextLength || IsRealSymbol(editor.Text.Substring(m.Index + iShift + sFind.Length, 1))))
                        {
                            bWholeWordEnd = true;
                        }
                    ...
                    iCountReplaced++;
                    editor.SelectionStart = m.Index + iShift; //重新選取內容
                    editor.SelectionEnd = m.Index + iShift + sFind.Length;
                    editor.ReplaceSelection(cboReplaceBox.Text); //置換內容

                    iShift += cboReplaceBox.TextLength - cboFindBox.TextLength;
                }

                SetLabelInfoEditor(sReplaceAllText + ": " + _sLangText.Replace("{qty}", iCountReplaced.ToString()), Color.Black);
            }

            editor.SelectionStart = iCurrentPos;
            editor.SelectionEnd = iCurrentPos;
            editor.ScrollCaret();

            Cursor = Cursors.Default;
        }

        private void cboFindBox_TextChanged(object sender, EventArgs e)
        {
            var bValue = false;
            var iCount = CountEditor(cboFindBox, false, (!btnMatchCase.Visible)); //cboFindBox_TextChanged, 統計出現次數，但不顯示
            cboFindBox.Tag = iCount.ToString();
            ...
            btnHighlightAllBox.Enabled = bValue;
            btnCountBox.Enabled = bValue;
            btnFindNextBox.Enabled = bValue;
            btnFindPreviousBox.Enabled = bValue;

            //20201111 cboReplaceBox.Text 允許空值，因為使用者可能要置換為空值
            if (bValue)
            {
                /*do nothing*/
            }
            ...
            btnReplace.Enabled = bValue;
            btnReplaceAll.Enabled = bValue;
        }

        private void cboReplaceBox_TextChanged(object sender, EventArgs e)
        {
            var bValue = cboFindBox.Text != "" && cboReplaceBox.Text != "";

            btnReplace.Enabled = bValue;
            btnReplaceAll.Enabled = bValue;
        }

        private void cboFindBox_DropDownOpened(object sender, EventArgs e)
        {
            
        }

        private void btnMatchWholeWordOnly_Click(object sender, EventArgs e)
        {
            //btnMatchWholeWordOnly2.Visible = btnMatchWholeWordOnly.Visible;
            //btnMatchWholeWordOnly.Visible = !btnMatchWholeWordOnly2.Visible;

            //20220220 上方寫法無作用，原因不明；改寫如下
            btnMatchWholeWordOnly.Visible = !btnMatchWholeWordOnly.Visible;
            btnMatchWholeWordOnly2.Visible = !btnMatchWholeWordOnly.Visible;
        }

        private void btnMatchCaseBox_Click(object sender, EventArgs e)
        {
            btnMatchCase2.Visible = btnMatchCase.Visible;
            btnMatchCase.Visible = !btnMatchCase2.Visible;
        }
        ...
        private void ExportToFile()
        {
            using (var myForm = new frmExportToFile())
            {
                ...
                Cursor = Cursors.Default;
                myForm.dtData = dtData;
                myForm.dtSchemaTable = _dtSchemaTable_Export;
        ...
        private void tmrlblInfo_Tick(object sender, EventArgs e)
        {
            ...
            if ((c1StatusBar1.Tag ?? string.Empty).ToString() == string.Empty) return;
        ...
        private void cboReplaceBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar != 13 || cboReplaceBox.Text.Trim() == "")
            {
                return;
            }

            UpdateReplaceList(); //cboReplaceBox_KeyPress
            cboReplaceBox.Focus();
        }
        ...
        private void txtMessage_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            //Select All
            _cMenuMessageEditor.Items[0].Enabled = txtMessage.Text != "";

            //Copy: 判斷是否有選取文字，決定功能表項目可不可用
            _cMenuMessageEditor.Items[2].Enabled = txtMessage.SelectedText != "";

            txtMessage.ContextMenuStrip = _cMenuMessageEditor;
            ...
            _cMenuMessageEditor.Show(txtMessage, new Point(e.X, e.Y));
        }
        ...
        private void editorSQLHistory_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            //Select All
            _cMenuSQLHistoryEditor.Items[0].Enabled = editorSQLHistory.Text != "";

            //Copy: 判斷是否有選取文字，決定功能表項目可不可用
            _cMenuSQLHistoryEditor.Items[2].Enabled = editorSQLHistory.SelectedText != "";
        ...
        private void SetDockingTabControl(bool bSendTabKey = false)
        {
            ...
            if ("`tabMessage`tabSQLHistory`".Contains("`" + c1DockingTab1.SelectedTab.Name + "`") == false)
            {
                ...
                {
                    if (!(ctl is C1TrueDBGrid)) continue;

                    var c1Grid = (C1TrueDBGrid)ctl;
                    var dt = (DataTable)c1Grid.DataSource;
            ...
            btnFindNextGrid.Enabled = cboFindGrid.Text != "";
            btnFindPreviousGrid.Enabled = cboFindGrid.Text != "";
            btnCountGrid.Enabled = cboFindGrid.Text != "";
            btnHighlightAllGrid.Enabled = cboFindGrid.Text != "";
            btnClearHighlightsGrid.Enabled = cboFindGrid.Text != "";
        ...
        private void CalculateCells()
        {
            ...
            lblSummaryValue.Text = dSummary.ToString("###,##0.################");
            lblCountValue.Text = iCount.ToString();
            lblAverageValue.Text = dSummary == 0 ? "0" : (dSummary / iCountAverage).ToString("###,##0.#####");
        }
        ...
        {
            var i = 2 + c1DockingTab1.TabPages.Cast<Control>().Count(ctrl => _sOriginal3TabName.Contains("`" + ctrl.Text + "`") == false); //_sDataGridTabName
            var page = new C1DockingTabPage {Text = _sDataGridTabName + i};
            c1DockingTab1.TabPages.Add(page);

            var c1Grid = new C1TrueDBGrid();
            ((ISupportInitialize)c1Grid).BeginInit();
            c1Grid.Name = "c1TrueDBGrid" + i;
            c1Grid.Dock = DockStyle.Fill;
            c1Grid.AllowUpdate = false;
            c1Grid.AllowUpdateOnBlur = false;
            c1Grid.AlternatingRows = true;
            page.Controls.Add(c1Grid);
            c1Grid.DataSource = _dtNull;
            ((ISupportInitialize)c1Grid).EndInit();
        }
        ...
        private C1TrueDBGrid GetWhichGrid()
        {
            ...
            foreach (Control tab in c1DockingTab1.TabPages)
            {
                ...
                foreach (var ctrlTab in tabPage.Controls)
                {
                    if (ctrlTab.GetType().Name != "C1TrueDBGrid") continue;

                    var sTemp = ((C1TrueDBGrid)ctrlTab).Name;

                    if (sTemp != "c1TrueDBGrid" + iQueryIndex) continue;
        ...
        private void pnlFindReplace_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button != MouseButtons.Left)
            {
                return;
        ...
        private void pnlFindReplace_MouseUp(object sender, MouseEventArgs e)
        {
            drag = false;
        }

        private void pnlFindReplace_MouseMove(object sender, MouseEventArgs e)
        {
            if (drag)
            {
                pnlFindReplace.Location = new Point(e.X + pnlFindReplace.Left - x, e.Y + pnlFindReplace.Top - y);
            }
        }

        private void lblFindAndReplace_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button != MouseButtons.Left)
            {
                return;
        ...
        private void lblFindAndReplace_MouseMove(object sender, MouseEventArgs e)
        {
            if (drag)
            {
                pnlFindReplace.Location = new Point(e.X + pnlFindReplace.Left - x, e.Y + pnlFindReplace.Top - y);
        ...
        private void lblFindAndReplace_MouseUp(object sender, MouseEventArgs e)
        {
            drag = false;
        }

        private void cboFindBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (cboFindBox.Text == "")
            {
                return;
        ...
        private static DialogResult InputBox(string sTitle, string sPromptText, string sPromptText2, int iX, int iY, ref string sValue, ref bool bAutoOpenFile)
        {
            ...
            sName = MyGlobal.GetLanguageString("&OK", "Global", "Global", "messagebox", "OK", "Text");
            btnOK.Text = sName;
            ...
            {
                btnOK.Enabled = txtInputBox.Text.Trim() != "";
            };
        ...
        private void c1TrueDBGrid1_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (!bCornerSelected || e.Button != MouseButtons.Left || e.X >= 20) return;
        ...
        private void HandleSqlExecuteErrorPosition_Oracle(string sSqlInfo, int iPos)
        {
            ...
            else
            {
                ...
                if (iFrom != -1 && iTo != -1 && iFrom < iTo) //iFrom < iTo：避開只有一個雙引號
                {
                    //可能情況：出現兩個單字，前後都有雙引號；故從第一個雙引號之後繼續找
                    iTo = sErrMsg.Substring(iFrom).IndexOf("\"", StringComparison.Ordinal) + iFrom;
                ...
                else
                {
                    ...
                    if (iFrom != -1 && iTo != -1 && iFrom - iTo > 1) //iFrom - iTo > 1：避開只有一個單引號
                    {
                        //錯誤訊息有明確指出哪個字串
                        sTempWord = sErrMsg.Substring(iFrom, iTo - iFrom);
                ...
                sTemp = (sExecutedResult == "" ? "" : sExecutedResult + "\r\n") + sErrCode + "ErrorMsg: " + sErrMsg + sErrHint + sTemp + (_sQueryTextParametersMapping == "" ? "" : "\r\n\r\n" + _sQueryTextParametersMapping);
                UpdateMessage(sTemp); //HandleSqlExecuteErrorPosition_Oracle
        ...
        private void HandleSqlExecuteErrorPosition_PostgreSQL(string sSqlInfo, int iPos)
        {
            ...
            else
            {
                ...
                if (iFrom != -1 && iTo != -1 && iFrom - iTo > 1) //iFrom - iTo > 1：避開只有一個雙引號
                {
                    //可能情況：出現兩個單字，前後都有雙引號；故從第一個雙引號之後繼續找
                    iTo = sErrMsg.Substring(iFrom).IndexOf("\"", StringComparison.Ordinal) + iFrom;
                            ...
                            if (bKeywordNotAppear == false)
                            {
                                ...
                                if (iErrLine > 0 && iErrLine <= parts.Length && parts[iErrLine - 1].IndexOf(sTempWord, StringComparison.Ordinal) != -1)
                                {
                                    ...
                                    foreach (var t in parts)
                                    {
                                        ...
                                        {
                                            var iLen = Encoding.Default.GetByteCount(t);
                                            iTemp9 += t.Length + 2;
                                        }
        ...
        private void HandleSqlExecuteErrorPosition_MySQL(string sSqlInfo, int iPos)
        {
            ...
            else
            {
                ...
                if (iFrom != -1 && iTo != -1 && iTo - iFrom > 1) //iTo - iFrom > 1：避開只有一個雙引號
                {
                    //可能情況：出現兩個單字，前後都有雙引號；故從第一個雙引號之後繼續找
                    iTo = sErrMsg.Substring(iFrom).IndexOf("'", StringComparison.Ordinal) + iFrom;
                ...
                else
                {
                    ...
                    if (iFrom != -1 && iTo != -1 && iTo - iFrom > 1) //iFrom - iTo > 1：避開只有一個單引號
                    {
                        //錯誤訊息有明確指出哪個字串
                        sTempWord = sErrMsg.Substring(iFrom, iTo - iFrom);
                ...
                sTemp = (sExecutedResult == "" ? "" : sExecutedResult + "\r\n") + sErrCode + (sErrCode == "" ? "ErrorMsg:" : "") + " " + sErrMsg + sErrHint + sTemp + (_sQueryTextParametersMapping == "" ? "" : "\r\n\r\n" + _sQueryTextParametersMapping);
                UpdateMessage(sTemp); //HandleSqlExecuteErrorPosition_MySQL
        ...
        private void QuerySchema()
        {
            ...
            try
            {
                if (MyGlobal.bShowColumnInfo)
                {
                    dtSchema.DefaultView.RowFilter = sFilter + "(SchemaName LIKE '" + txtSchemaFilter.Text.Trim().Replace("*", "%") + "%' OR Schema_Browser LIKE '" + txtSchemaFilter.Text.Trim().Replace("*", "%") + "%')";
                }
                else
                {
                    dtSchema.DefaultView.RowFilter = sFilter + "Schema_Browser LIKE '" + txtSchemaFilter.Text.Trim().Replace("*", "%") + "'";
                }
            }
            catch (Exception ex)
            {
        ...
        private void ArrangeSchemaData4CopyPaste(string sMode)
        {
            ...
            else
            {
                ...
                {
                    DataTable dt = (DataTable)c1GridSchemaBrowser.DataSource;
                    //NG: sTemp = c1GridSchemaBrowser[vr.DataRowIndex, "Schema_Browser"].ToString();
        ...
        private void c1GridSchemaBrowser_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (iDisplayRowIndex == -1) return; //Exclude row headers

            //20211124 右鍵選單，要再重新寫過
            //return;
            ...
            if (c1GridSchemaBrowser.Splits[_iii].Rows[iDisplayRowIndex].RowType == RowTypeEnum.CollapsedGroupRow || c1GridSchemaBrowser.Splits[_iii].Rows[iDisplayRowIndex].RowType == RowTypeEnum.ExpandedGroupRow)
            {
                ...
                if (sSchemaType != sSchemaType2 && sSchemaType2 != "")
                {
                    if (MyGlobal.sDataSource == @"Oracle" && iLevel > 1)
                    {
                    ...
                    else if (MyGlobal.sDataSource == "PostgreSQL" && iLevel > 2)
                    {
                        var sTempSchemaObject1 = dtSchema0.Select($"SchemaType = '{sSchemaType}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                        var sTempSchemaObject2 = dtSchema0.Select($"SchemaType = '{sSchemaType2}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                    ...
                    else if (MyGlobal.sDataSource == "SQL Server" && iLevel > 2)
                    {
                        var sTempSchemaObject1 = dtSchema0.Select($"SchemaType = '{sSchemaType}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                        var sTempSchemaObject2 = dtSchema0.Select($"SchemaType = '{sSchemaType2}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                    ...
                    else if (MyGlobal.sDataSource == "MySQL" && iLevel > 2)
                    {
                        var sTempSchemaObject1 = dtSchema0.Select($"SchemaType = '{sSchemaType}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                        var sTempSchemaObject2 = dtSchema0.Select($"SchemaType = '{sSchemaType2}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                    ...
                    else if (MyGlobal.sDataSource == "SQLite" && iLevel > 2)
                    {
                        var sTempSchemaObject1 = dtSchema0.Select($"SchemaType = '{sSchemaType}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                        var sTempSchemaObject2 = dtSchema0.Select($"SchemaType = '{sSchemaType2}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                    ...
                    else if (MyGlobal.sDataSource == "SQLCipher" && iLevel > 2)
                    {
                        var sTempSchemaObject1 = dtSchema0.Select($"SchemaType = '{sSchemaType}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                        var sTempSchemaObject2 = dtSchema0.Select($"SchemaType = '{sSchemaType2}' And SchemaName = '" + ((GroupRow)vr).GroupedText + "'").FirstOrDefault()?["SchemaObject"].ToString();
                ...
                if (MyGlobal.sDataSource == "PostgreSQL" && iLevel > 2)
                {
        ...
        private void cboFindBox_BeforeDropDownOpen(object sender, System.ComponentModel.CancelEventArgs e)
        {
            LoadFindList("Editor", cboFindBox); //cboFindBox_BeforeDropDownOpen
        }

        private void cboReplaceBox_BeforeDropDownOpen(object sender, System.ComponentModel.CancelEventArgs e)
        {
            UpdateReplaceList(); //cboReplaceBox_BeforeDropDownOpen
        }
        ...
        {
            LoadFindList("Editor", cboFind); //cboFind_BeforeDropDownOpen
        }
        ...
        private void AutoResizeGridColumnWidth()
        {
            ...
            if (!MyLibrary.bEnableAutoReplace) return;
        ...
        {
            private const string RegexSpecSymbolsPattern = @"[\^\$\[\]\(\)\.\\\*\+\|\?\{\}]";
            ...
            public override CompareResult Compare(string fragmentText)
            {
                ...
                return Regex.IsMatch(Text, "\\b" + pattern, RegexOptions.IgnoreCase) ? CompareResult.Visible : CompareResult.Hidden;
            }
        }
        ...
        private class InsertSpaceSnippet : AutocompleteItem
        {
            ...
            public override CompareResult Compare(string fragmentText)
            {
                ...
                return Text != fragmentText ? CompareResult.Visible : CompareResult.Hidden;
            }
        ...
        private class InsertEnterSnippet : AutocompleteItem
        {
            ...
            public override CompareResult Compare(string fragmentText)
            {
                ...
                for (var i = Parent.Fragment.Start - 1; i >= 0; i--)
                {
                    if (text[i] == '\n')
                    {
                        break;
