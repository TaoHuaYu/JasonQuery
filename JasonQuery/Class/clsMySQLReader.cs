...
namespace JasonQuery
{
    public class clsMySQLReader
    {
        ...
        private static MySqlConnection _mConn;
        private MySqlTransaction txn;

        private string sName;
        private MySqlCommand oCommand;
        private MySqlDataAdapter oDataAdapter;
        ...
        public string ConnectTo(string strConnectionString)
        {
            ...
            _mConn = new MySqlConnection(strConnectionString);
        ...
        public void ExecuteQueryPaged(object sqlQuery)
        {
            ...
            catch (MySqlException ex)
            {
                ...
                if (sMessage.StartsWith("Unknown column '") && sMessage.IndexOf(" in 'where clause'", StringComparison.Ordinal) != -1)
                {
                    iFrom = sMessage.Replace(" in 'where clause'", "").IndexOf("'", StringComparison.Ordinal) + 1;
                    iTo = sMessage.Replace(" in 'where clause'", "").LastIndexOf("'", StringComparison.Ordinal);
                }
                else if (sMessage.StartsWith("Table") && sMessage.IndexOf("doesn't exist", StringComparison.Ordinal) != -1)
                {
                    iFrom = sMessage.Replace("doesn't exist", "").IndexOf("'", StringComparison.Ordinal) + 1;
                    iTo = sMessage.Replace("doesn't exist", "").LastIndexOf("'", StringComparison.Ordinal);
                }
                ...
                if (iFrom != -1 && iTo != -1)
                {
                    ...
                    if (sTempWord.IndexOf(" limit 0,", StringComparison.Ordinal) != -1)
                    {
                        sPagedWord = sTempWord.Substring(sTempWord.IndexOf(" limit 0,", StringComparison.Ordinal));
                        sMessage = sMessage.Replace(sPagedWord, "");

                        sTempWord = sTempWord.Substring(0, sTempWord.IndexOf(" limit 0,", StringComparison.Ordinal));
                    }
                    else if (sTempWord.IndexOf("\r\nlimit 0,", StringComparison.Ordinal) != -1)
                    {
                        sPagedWord = sTempWord.Substring(sTempWord.IndexOf("\r\nlimit 0,", StringComparison.Ordinal));
                        sMessage = sMessage.Replace(sPagedWord, "");

                        sTempWord = sTempWord.Substring(0, sTempWord.IndexOf("\r\nlimit 0,", StringComparison.Ordinal));
                    }
        ...
        public MySqlDataReader ExecuteQueryPaged100Rows(string sSql, int iStartRow, int iPageLength)
        {
        ...
        public void ExecuteNonQuery(object sqlQuery)
        {
                ...
                if (script.Statements.Count > 1)
                {
                    ...
                    for (i = 0; i < iBatchRunQty; i++)
                    {
                        ...
                        if (sResult == "Use script - Database changed!")
                        {
                            ...
                            MyGlobal.sGlobalTemp2 = "mysqlswitchdatabase" + MyGlobal.sSeparator + sAccessibleDescription + ";" + sDB;
                            MyGlobal.sGlobalTemp3 = "mysqlswitchdatabasebyusing`" + sDB;
                        }
            ...
            catch (MySqlException ex)
            {
                if (iBatchRunQty > 0)
                {
                    Thread.Sleep(1);
                    Application.DoEvents();
                    MyGlobal.iProgressInsertInto = iBatchRunQty;
                }
                ...
                var sErrorCode = "Error: " + ex.Code + " (" + ex.SqlState + ")";
                var sErrorHint = "";
                var sMessage = ex.Message;

                //傳回 SQL 錯誤的字串位置、錯誤訊息，由「該 QueryEditor」將錯誤的字串標示波浪底線
                iSelectionStart += iPosOffset; //錯誤的定位點
                MyGlobal.sGlobalTemp = "sqlexecuteerrorpos" + MyGlobal.sSeparator + sAccessibleDescription + ";" + sExecutedResult + MyGlobal.sSeparator5 + sErrorCode + MyGlobal.sSeparator5 + sMessage + MyGlobal.sSeparator5 + sErrorHint + MyGlobal.sSeparator5 + iSelectionStart + MyGlobal.sSeparator5 + sSqlExecuted;
